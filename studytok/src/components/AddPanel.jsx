...
export default function AddPanel({ onClose, onSave, onSaveMany, showToast }) {
  ...
  async function generate() {
    if (!sourceText.trim()) { showToast('⚠️', 'Falta texto', 'Sube un PDF o pega texto'); return }
    setLoadingAI(true)
    setAiCards([])
    try {
      const res = await fetch('/api/generate', {
        ...
        body: JSON.stringify({ text: sourceText, count: cardCount, mode: 'cards' }),
      })
      const data = await res.json()
      if (data.error) throw new Error(data.error)
      setAiCards(data.cards || [])
    } catch (e) {
      showToast('❌', 'Error de IA', e.message)
    }
    setLoadingAI(false)
  }
  ...
  function fillForm(card) {
    ...
    if (card.options) {
      setOpts(card.options.map(o => o.text).concat(['', '', '', '']).slice(0, 4))
      setCorrectIdx(card.options.findIndex(o => o.correct))
    }
  ...
  function save() {
    ...
    if (type === 'concept' || type === 'fact') {
      ...
      card.explanation = explanation.trim()
    } else if (type === 'memory') {
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 200, display: 'flex', alignItems: 'flex-end' }}>
      ...
      <div style={{
        ...
        <button onClick={onClose} style={{
          ...
          fontSize: '1rem', width: 32, height: 32, borderRadius: '50%', cursor: 'pointer',
        }}>✕</button>

        <div style={{ fontWeight: 900, fontSize: '1.05rem', color: '#1e1b4b', marginBottom: 4 }}>Nueva tarjeta ✏️</div>
        <div style={{ fontSize: '0.8rem', color: '#6b7280', marginBottom: 18 }}>Crea manualmente o genera desde PDF con IA</div>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
          {[['ai', '✨ Desde PDF / IA'], ['manual', '✏️ Manual']].map(([t, label]) => (
            <button key={t} onClick={() => setTab(t)} style={{
              ...
              border: `1.5px solid ${tab === t ? 'rgba(124,58,237,0.4)' : 'rgba(0,0,0,0.1)'}`,
              background: tab === t ? 'rgba(124,58,237,0.1)' : '#fff',
        ...
        {tab === 'ai' && (
          <div>
            ...
            <div
              ...
              style={{
                border: '2px dashed rgba(124,58,237,0.35)', borderRadius: 16,
                padding: '20px 16px', textAlign: 'center', cursor: 'pointer',
                marginBottom: 14, transition: 'all 0.15s',
                background: pdfName ? 'rgba(124,58,237,0.06)' : 'transparent',
              }}>
              {loadingPDF ? (
                <div style={{ color: '#7c3aed', fontSize: '0.9rem', fontWeight: 700 }}>Procesando PDF...</div>
              ) : pdfName ? (
                <>
                  <div style={{ fontSize: '1.8rem', marginBottom: 4 }}>📄</div>
                  <div style={{ color: '#7c3aed', fontSize: '0.85rem', fontWeight: 700 }}>{pdfName}</div>
                  <div style={{ color: '#9ca3af', fontSize: '0.72rem', marginTop: 3 }}>Texto extraído ✓ · Toca para cambiar</div>
                </>
              ) : (
                <>
            ...
            <div style={{ marginBottom: 14 }}>
              <label style={lbl}>O pega texto / apuntes directamente</label>
              <textarea value={sourceText} onChange={e => setSourceText(e.target.value)}
                placeholder="Pega aquí tus apuntes, definición, artículo de ley..."
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                {[1, 3, 5, 8].map(n => (
                  <button key={n} onClick={() => setCardCount(n)} style={{
                    ...
                    background: cardCount === n ? 'rgba(124,58,237,0.12)' : '#fff',
                    border: `1.5px solid ${cardCount === n ? '#7c3aed' : 'rgba(0,0,0,0.1)'}`,
                    color: cardCount === n ? '#6d28d9' : '#6b7280',
                  }}>{n}</button>
                ))}
              </div>
            </div>

            <button onClick={generate} disabled={loadingAI || !sourceText.trim()} style={{
              width: '100%', padding: '12px', borderRadius: 12, border: 'none',
              background: 'linear-gradient(135deg,#7c3aed,#be185d)', color: '#fff',
              fontFamily: 'inherit', fontWeight: 900, fontSize: '0.9rem',
              cursor: loadingAI || !sourceText.trim() ? 'not-allowed' : 'pointer',
              opacity: loadingAI || !sourceText.trim() ? 0.5 : 1,
            }}>
            ...
            {aiCards.length > 0 && (
              <div style={{ marginTop: 18 }}>
                ...
                {aiCards.map((c, i) => {
                  ...
                  return (
                    <div key={i} onClick={() => fillForm(c)} style={{
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 5 }}>
                        <span style={{
                          ...
                          color: p.badgeText, background: p.badge,
                          border: `1px solid ${p.badgeBorder}`, padding: '2px 8px', borderRadius: 20,
                        }}>{TYPE_LABEL[c.type]}</span>
                        <span style={{ fontFamily: 'Space Mono, monospace', fontSize: '0.6rem', color: '#9ca3af' }}>{c.subject}</span>
                      </div>
                      <div style={{ fontWeight: 700, fontSize: '0.88rem', color: p.title, marginBottom: 3 }}>{c.title}</div>
                      <div style={{ fontSize: '0.78rem', color: p.body, lineHeight: 1.5, opacity: 0.8 }}>
                        {(c.body || c.question || c.front || '').slice(0, 100)}
                        {(c.body || c.question || c.front || '').length > 100 ? '...' : ''}
                      </div>
                      <div style={{ fontSize: '0.72rem', color: p.accent, fontWeight: 700, marginTop: 6 }}>Editar y guardar →</div>
                    </div>
                  )
                })}
                <button onClick={() => { onSaveMany(aiCards); setAiCards([]); onClose() }} style={{
                  width: '100%', padding: '11px', borderRadius: 12, border: 'none',
                  background: 'linear-gradient(135deg,#7c3aed,#be185d)', color: '#fff',
                  fontFamily: 'inherit', fontWeight: 900, fontSize: '0.88rem', cursor: 'pointer',
                }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 18, flexWrap: 'wrap' }}>
              {Object.keys(TYPE_LABEL).map(t => (
                <button key={t} onClick={() => setType(t)} style={{
                  ...
                  background: type === t ? TYPE_COLORS[t].bg : '#fff',
                  border: `1.5px solid ${type === t ? TYPE_COLORS[t].a : 'rgba(0,0,0,0.1)'}`,
                  color: type === t ? TYPE_COLORS[t].a : '#6b7280',
            ...
            <div style={{ marginBottom: 12 }}>
              ...
              <input value={subject} onChange={e => setSubject(e.target.value)}
                placeholder="Ej: Derecho Tributario" style={inp} />
            </div>
            <div style={{ marginBottom: 12 }}>
              <label style={lbl}>Título</label>
              <input value={title} onChange={e => setTitle(e.target.value)}
                placeholder="Título de la tarjeta" style={inp} />
            </div>
            ...
            {(type === 'concept' || type === 'fact') && (
              <div style={{ marginBottom: 12 }}>
                ...
                <textarea value={body} onChange={e => setBody(e.target.value)}
                  placeholder="Explica el concepto en 2-3 oraciones..."
                  style={{ ...inp, minHeight: 80 }} />
              </div>
            )}
              ...
              <>
                <div style={{ marginBottom: 12 }}>
                  ...
                  <input value={question} onChange={e => setQuestion(e.target.value)}
                    placeholder="¿Cuál es...?" style={inp} />
                </div>
                <div style={{ marginBottom: 12 }}>
                  <label style={lbl}>Opciones — marca el círculo de la correcta</label>
                  {opts.map((o, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 7 }}>
                      <input type="radio" name="correct" checked={correctIdx === i}
                        onChange={() => setCorrectIdx(i)}
                        style={{ accentColor: '#be185d', width: 16, height: 16, flexShrink: 0 }} />
                      <span style={{ fontFamily: 'Space Mono, monospace', fontSize: '0.6rem', color: '#9ca3af' }}>
                        {String.fromCharCode(65 + i)}
                      </span>
                      <input value={o} onChange={e => { const n = [...opts]; n[i] = e.target.value; setOpts(n) }}
                        placeholder={`Opción ${String.fromCharCode(65 + i)}${i > 1 ? ' (opcional)' : ''}`}
                        style={{ ...inp, flex: 1, padding: '9px 11px' }} />
                    </div>
                  ))}
                </div>
                <div style={{ marginBottom: 12 }}>
                  <label style={lbl}>Explicación (se muestra si fallan)</label>
                  <textarea value={explanation} onChange={e => setExplanation(e.target.value)}
                    placeholder="¿Por qué es correcta esa opción?"
            ...
            {type === 'memory' && (
              <>
                <div style={{ marginBottom: 12 }}>
                  <label style={lbl}>Pregunta / Frente</label>
                  <textarea value={front} onChange={e => setFront(e.target.value)}
                    placeholder="Pregunta o concepto que aparece al frente"
                    style={{ ...inp, minHeight: 65 }} />
                </div>
                <div style={{ marginBottom: 12 }}>
                  <label style={lbl}>Respuesta / Reverso</label>
                  <textarea value={back} onChange={e => setBack(e.target.value)}
                    placeholder="Respuesta que se revela al girar"
                    style={{ ...inp, minHeight: 65 }} />
                </div>
              </>
            )}
